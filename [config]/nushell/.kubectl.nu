...
def "nu-complete kubectl logs" [] {
    ^kubectl __complete logs "" err> NUL | lines | drop
}
...
export extern "kubectl logs" [
    ...
    --insecure-skip-tls-verify-backend # Skip verifying the identity of the kubelet that logs are requested from.
...
def "nu-complete kubectl" [] {
    ^kubectl __complete "" err> nul | lines | each {|| split words | first 1 | str join }
}
...
export extern "kubectl" [
  ...
  logs?: string@"nu-complete kubectl logs"
